...
- **공유 컴포넌트 일관성 (No-Duplicate UI 규칙)** — 같은 시각/UX 의 컴포넌트는
  ...
  | 3단계 사용률 상태 점 (초록/주황/빨강, 이모지 대체) | `@/components/ui/StatusDot` | UsageLimitPanel, AccountLimits |
  ...
  - **한도 표기 통일 (2026-07-14): 숫자·게이지 채움·신호색 전부 = 사용률** (트레이·한도 패널·계정 한도 공통). `pickQuotaSignal`/`QuotaSegBar`/`StatusDot` 입력은 "사용률"(0..1): <0.4 lime / 0.4~0.7 **orange** / ≥0.7 coral (많이 쓸수록 위험색). `StatusDot` prop 은 `used`(정렬은 `minRemaining` 로 여유 많은 순). 트레이(`tray_render.rs::band_color`)도 동일 임계값(사용률 40/70)·systemOrange, 사용률을 **배터리 셀**(중립 트랙 + 상태색 채움 + nub + 안에 n%)로 표시 — 옆 메뉴바 아이콘과 톤 통일.
