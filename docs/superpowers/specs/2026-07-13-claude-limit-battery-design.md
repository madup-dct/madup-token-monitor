...
1. 5h / 7d(모든 모델) / Fable 주간 한도를 **잔여 %(배터리)** 로 표기.
2. 색상: 잔여 ≥70% 초록 / ≥30% 노랑 / <30% 빨강. 여유(초록) 상태도 상시 표시.
...
- **표현 방식: 상태 문자열 전체를 이미지로 렌더해 트레이 아이콘으로 교체**
  ...
  ```
  [로고] ●5h 92  ●7d 72  ●F 52   (● = 이미지 내 작은 원, 3단계 상태색)
  ```

  - 한도 토글 on 이면: 로고 + 점 + 잔여 숫자(+ 비용 토글 on 시 `$12`)를 **하나의
...
- `LimitRow` 를 배터리 의미로 전환:
  ...
  - 숫자: **잔여 %** (`100 - utilization`, 정수 반올림) — 예: "잔여 48%".
  - 세그먼트 바(`QuotaSegBar`): 잔여만큼 채움 (배터리 게이지).
...
- 라우트/네비 신설. 테이블 한 줄 = 계정 하나:
  ...
  - 각 창은 미니 배터리 바(`QuotaSegBar` 재사용) + 잔여 %.
