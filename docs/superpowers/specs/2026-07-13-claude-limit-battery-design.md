...
2. 색상: **사용률** 기준 <40% 초록 / 40~70% 주황(systemOrange) / ≥70% 빨강
   (2026-07-14 확정 — 많이 쓸수록 위험색). 트레이는 사용률을 **배터리 셀** 아이콘으로 표시
   (중립 트랙 + 상태색 채움 + nub + 안에 n% 텍스트 — 옆 메뉴바 아이콘과 톤 통일).
   계정 한도 페이지는 창(5h/7d/Fable)별 초기화 시각을 각자 표기.
